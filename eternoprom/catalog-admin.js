(function(){
  ...
  var GOD_FLAG_KEY = 'epm_open_god_mode';
  ...
  function wantsGodMode(){
    try {
      var query = new URLSearchParams(location.search || '');
      return query.get('god') === '1' || localStorage.getItem(GOD_FLAG_KEY) === '1';
    } catch (e) {
      return false;
    }
  }
  function clearGodModeIntent(){
    try { localStorage.removeItem(GOD_FLAG_KEY); } catch (e) {}
  }
  ...
  function injectStyles(){
    ...
    style.textContent = '\
      ...
      .epm-god-welcome{position:fixed;left:18px;bottom:18px;z-index:99989;max-width:min(420px,calc(100vw - 110px));background:#fff;border:1px solid #dfe7f5;border-radius:18px;box-shadow:0 14px 34px rgba(0,25,82,.16);padding:16px 18px;font-family:Inter,sans-serif;}\
      .epm-god-welcome h4{margin:0 0 8px;font-size:15px;color:#001952;}\
      .epm-god-welcome p{margin:0 0 12px;font-size:12px;line-height:1.6;color:#475569;}\
      .epm-god-welcome--warning{border-color:#fecaca;background:#fff7f7;}\
  ...
  function showWelcomeBox(title, message, actionsHtml, warning){
    var existing = document.getElementById('epmGodWelcome');
    if (existing) existing.remove();
    document.body.insertAdjacentHTML('beforeend',
      '<div id="epmGodWelcome" class="epm-god-welcome' + (warning ? ' epm-god-welcome--warning' : '') + '">' +
      '<h4>' + esc(title) + '</h4><p>' + esc(message) + '</p><div class="epm-god-btns">' + actionsHtml + '</div></div>'
    );
    var closeBtn = document.getElementById('epmGodWelcomeClose');
    if (closeBtn) closeBtn.onclick = function(){ var box = document.getElementById('epmGodWelcome'); if (box) box.remove(); };
    var openBtn = document.getElementById('epmGodWelcomeOpen');
    if (openBtn) openBtn.onclick = function(){ var toggle = document.getElementById('epmGodToggle'); if (toggle) toggle.click(); };
  }
  ...
  function ensureGodUi(){
    ...
    if (wantsGodMode()) {
      state.mode = true;
      document.getElementById('epmGodPanel').classList.add('open');
      showWelcomeBox('Режим бога включён', 'Панель уже открыта справа. Добавьте карточку в форме, затем нажмите «Сохранить всё».', '<button id="epmGodWelcomeClose" class="epm-god-btn ghost">Понятно</button>', false);
      clearGodModeIntent();
    }
  }
  ...
  fetchCatalogData().then(function(data){
    ...
    if (wantsGodMode() && !isAdmin()) {
      showWelcomeBox('Почему вы видите обычный каталог', 'Для режима бога нужен активный вход в админку. Сначала войдите в admin.html под администратором, затем снова откройте эту страницу.', '<a class="epm-god-btn primary" href="/eternoprom/admin/index.html">Войти в админку</a><button id="epmGodWelcomeClose" class="epm-god-btn ghost">Закрыть</button>', true);
      clearGodModeIntent();
    }
    loadAdminState();
    renderGodMode();
