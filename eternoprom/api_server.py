...
def upload_catalog_image(req: ImageUploadRequest, token=Depends(get_token)):
    suffix = Path(req.filename or "image.bin").suffix.lower() or ".bin"
    ...
    try:
        content = base64.b64decode(req.content_base64.encode("utf-8"), validate=True)
    except Exception:
        raise HTTPException(status_code=400, detail="Invalid image payload")
    target.write_bytes(content)
    return {"url": f"/uploads/catalog/{filename}", "name": req.filename}
