"""
api_server.py — ЭТЕРНОПРОМ Backend
====================================
...
<div style="background:#001952;padding:16px 24px;border-radius:8px 8px 0 0;">
  <h2 style="color:#fff;margin:0;font-size:18px;">Новый заказ #{entry['id']} — ЭТЕРНОПРОМ</h2>
</div>
<div style="background:#fff;border:1px solid #e8ecf4;border-top:none;border-radius:0 0 8px 8px;padding:20px 24px;">
  ...
  <div style="margin-top:20px;padding:12px;background:#f5f7fa;border-radius:8px;font-size:12px;color:#8a96ae;">Дата: {entry.get('created_at','')}</div>
    ...
    send_email(f"[ЭТЕРНОПРОМ] Новый заказ #{entry['id']} от {entry.get('name','Клиент')}", body)

def email_lead(entry: dict):
    body = f"""<div style="font-family:Arial,sans-serif;max-width:500px;margin:0 auto;">
<div style="background:#8B1A1A;padding:14px 20px;border-radius:8px 8px 0 0;"><h2 style="color:#fff;margin:0;font-size:16px;">Запрос цены #{entry['id']} — ЭТЕРНОПРОМ</h2></div>
<div style="background:#fff;border:1px solid #e8ecf4;border-top:none;border-radius:0 0 8px 8px;padding:18px 20px;">
  ...
  <tr><td style="color:#8a96ae;padding:4px 0;">Дата:</td><td>{entry.get('created_at','')}</td></tr>
    ...
    send_email(f"[ЭТЕРНОПРОМ] Запрос цены: {entry.get('product','товар')}", body)

def email_callback(entry: dict):
    body = f"""<div style="font-family:Arial,sans-serif;max-width:480px;margin:0 auto;">
<div style="background:#22c55e;padding:14px 20px;border-radius:8px 8px 0 0;"><h2 style="color:#fff;margin:0;font-size:16px;">Заказ звонка #{entry['id']} — ЭТЕРНОПРОМ</h2></div>
<div style="background:#fff;border:1px solid #e8ecf4;border-top:none;border-radius:0 0 8px 8px;padding:18px 20px;">
  <table style="font-size:13px;" width="100%">
  ...
  <tr><td style="color:#8a96ae;padding:4px 0;">Дата:</td><td>{entry.get('created_at','')}</td></tr>
    ...
    send_email(f"[ЭТЕРНОПРОМ] Заказ звонка от {entry.get('name','Клиента')}: {entry.get('phone','')}", body)

# ─── FastAPI app ───────────────────────────────────────────────────────────────

app = FastAPI(title="ЭТЕРНОПРОМ CRM API", version="1.0.0")
...
@app.get("/")
def root():
    return {"status": "ok", "service": "ЭТЕРНОПРОМ CRM API"}
...
def test_email_route(token=Depends(get_token)):
    result = send_email(
        "[ЭТЕРНОПРОМ] Тестовое письмо",
        "<p>Если вы получили это письмо — SMTP настроен корректно.</p><p>Система CRM ЭТЕРНОПРОМ работает.</p>"
    )
    if result:
        return {"status": "sent"}
